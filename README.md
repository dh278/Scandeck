...
1. シャッターボタンをタップすると端末純正のカメラアプリが開くので撮影(高解像度で撮れるようにするための仕様。画面内の映像はあくまで構図確認用のプレビュー)。または左上の🖼ボタンで既存の写真を選択(複数選択も可)
...
| 自動輪郭検出 | Canny法(しきい値2種)+大津の二値化による輪郭検出に加え、直線検出(Hough変換)で上下左右の境界線の交点から四隅を推定する方式も組み合わせたアンサンブル方式(書類の端が写真のフチにかかっている場合などにも対応) |
...
| 撮影 | 端末純正カメラアプリ(`<input capture>`)を呼び出して撮影。iOS Safariは高解像度スチル撮影用のImageCapture APIに非対応なため、動画プレビューからの切り出しではなくOSのフル解像度撮影を使うことでA3等の大きい書類でも文字が潰れにくくしている |
